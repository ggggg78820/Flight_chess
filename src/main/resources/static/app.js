...
(() => {
  ...
  */
   ...
   * idx：這次實際在移動的是 mover 的第幾架飛機——被砲塔打回機坪的一定是「這一架」，
   *      不是 mover 這一方目前跑最前面的飛機（如果一方同時有多架飛機在跑道上，「正在移動的」
   *      跟「跑最前面的」不一定是同一架，用最前面那架來代表「被打中的是誰」會跟畫面上
   *      「明明是這架飛機經過砲塔」的直覺不一致，是很容易在展示時被看穿的邏輯錯誤）。
   * 依序檢查移動路徑上每一格，只要找到敵方「還沒失效」的塔（!t.used）就套用效果、並呼叫 markTowerUsed()
   * 讓這座塔立刻失效——每座塔的技能只能觸發這麼一次：
   *   砲塔（cannon）：把正在移動的這架飛機直接送回機坪（位置設回 -1），
   ...
   */
  function applyTowerEffects(mover, idx, from, to) {
   ...
   * 同時把整局收集到的 state.towers（雙方建過的每一座塔：類型、棋盤位置、擁有者、是否觸發過）
   * 一起送過去，後端會依此逐一建立 GameTower 關聯紀錄，並更新這個使用者在 users 資料表裡的
   * 累計勝敗場次（實際邏輯都在 GameService.endGame() 裡）。
   *
   * 注意 request body 裡故意沒有 userId：這局是誰玩的，後端一律從登入 session 讀出來
   * （見 GameController/SessionUtil），不再相信前端宣稱的身分。如果目前是離線模式
   * （currentUser 是 null，代表還沒登入），就直接跳過、不呼叫後端，這一局的結果只會留在
   * 瀏覽器記憶體裡（state.user 本地端的數字），不會被儲存；真的呼叫了但後端說「還沒登入」
   * （401）時也會落到下面的 catch，效果一樣是不儲存。
   *
